...
for path in sorted(ART.glob('*.obj')):
    ...
    task=u.AssetImportTask();task.filename=str(path);task.destination_path='/Game/Lunar/V2/Meshes';task.destination_name=path.stem
    ...
    mesh=lib.load_asset('/Game/Lunar/V2/Meshes/'+path.stem)
...
for path in [ART/'T_Earth_Clouds.jpg']:
    ...
    task=u.AssetImportTask();task.filename=str(path);task.destination_path='/Game/Lunar/V2/Textures';task.destination_name=path.stem
    task.automated=True;task.replace_existing=True;task.save=True
    assets.import_asset_tasks([task])
    tex=lib.load_asset('/Game/Lunar/V2/Textures/'+path.stem)
...
(ROOT/'Reports/v2-import.json').write_text(json.dumps(report,indent=2))
u.log('LUNAR_V2_IMPORT_READY '+str(report))
