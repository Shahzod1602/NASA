...
def material(name,unlit=False,two=False):
    path='/Game/Lunar/V3/Materials/'+name
    m=lib.load_asset(path) if lib.does_asset_exist(path) else assets.create_asset(name,'/Game/Lunar/V3/Materials',u.Material,u.MaterialFactoryNew())
...
base=custom(reg,noise+'float c=.38+g.f(P*.17)*.22+g.f(P*.017)*.32+g.f(P*.0009)*.58; return float3(.115,.113,.109)*c;', [('P',pos)])
out(reg,base,u.MaterialProperty.MP_BASE_COLOR)
normal=custom(reg,noise+'float h=g.f(P*.14)+g.f(P*.022)*2.5; float dx=g.f((P+float3(.7,0,0))*.14)+g.f((P+float3(.7,0,0))*.022)*2.5-h; float dy=g.f((P+float3(0,.7,0))*.14)+g.f((P+float3(0,.7,0))*.022)*2.5-h; float3 bump=float3(-dx,-dy,0)*5; return normalize(N+bump-N*dot(N,bump));',[('P',pos),('N',node(reg,u.MaterialExpressionVertexNormalWS))])
...
basalt=material('M_FracturedBasalt')
bp=node(basalt,u.MaterialExpressionWorldPosition)
bc=custom(basalt,noise+'float c=.55+g.f(P*.035)*.7; return float3(.068,.066,.063)*c;',[('P',bp)])
out(basalt,bc,u.MaterialProperty.MP_BASE_COLOR)
out(basalt,scalar(basalt,.94),u.MaterialProperty.MP_ROUGHNESS)
out(basalt,scalar(basalt,.10),u.MaterialProperty.MP_SPECULAR)
save(basalt)
...
prefixes=['Lunar surface','Crater dark basin','Crater regolith rim','Regolith boulder','Distant lunar ridge','Earth over horizon','Distant star','V2_','V3_']
removed=0
for a in list(actors.get_all_level_actors()):
    ...
    elif label=='Soft readability fill':a.light_component.set_intensity(.065)
...
def mesh(name,asset,p=(0,0,0),scale=(1,1,1),mat=None,collision=True,rot=(0,0,0)):
    a=actors.spawn_actor_from_class(u.StaticMeshActor,u.Vector(*p),u.Rotator(*rot));a.set_actor_label('V3_'+name)
    ...
    if not collision:c.set_cast_shadow(name in ['Fine surface debris','Basalt fragment'])
    if name=='Fine surface debris':c.set_editor_property('ld_max_draw_distance',6500)
    return a

terrain=mesh('Continuous crater terrain','/Game/Lunar/V3/Meshes/SM_LunarTerrain',mat=reg)
horizon=mesh('Distant crater highlands','/Game/Lunar/V3/Meshes/SM_LunarHorizon',mat=reg,collision=False)
# Keep distant horizon beneath the local playable basin.
horizon.set_actor_location(u.Vector(0,0,-1500),False,False)

data=json.loads((ROOT/'SourceArt/LunarV3/geometry.json').read_text())
...
for i in range(1000):
    ...
    a=mesh('Basalt fragment',f'/Game/Lunar/V3/Meshes/SM_Basalt_{i%6}',(x,y,ground(x,y)+size*12),(size,size*.85,size),reg if i%3==0 else basalt,size>.8,(rng.uniform(-25,25),rng.uniform(0,360),rng.uniform(-20,20)))
    a.static_mesh_component.set_editor_property('ld_max_draw_distance',24000)
for i in range(850):
    x,y=rng.uniform(-4200,6500),rng.uniform(-3300,3500)
    size=rng.uniform(.045,.33)
    mesh('Fine surface debris',f'/Game/Lunar/V3/Meshes/SM_Basalt_{i%6}',(x,y,ground(x,y)+size*4),(size,size,size*.6),basalt if i%3 else reg,False,(0,rng.uniform(0,360),0))
...
lib.save_directory('/Game/Lunar/V3',only_if_is_dirty=False,recursive=True)
report={'removed_prototype_actors':removed,'actors':len(actors.get_all_level_actors()),'catalog_stars':5044,'terrain_triangles':889778,'earth_angular_diameter_degrees':2,'terrain':'procedural, not surveyed NASA terrain','planets':'artist-placed distant points; no ephemeris simulation','visibility':'star brightness enhanced for game readability'}
(ROOT/'Reports/lunar-v3.json').write_text(json.dumps(report,indent=2))
u.log('LUNAR_V3_READY '+str(report))
